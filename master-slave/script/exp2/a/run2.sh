...
for DATA_FILE in "${DATA_FILES[@]}"; do
    ...
    for PUSH_STRATEGY in "${PUSH_STRATEGIES[@]}"; do
        for ((x = 1; x <= RUNS; x++)); do
            ...
            if [ -f "${OUT_FILE}" ]; then
                echo "=== ${DATA_FILE_NAME} (pool_size=${POOL_SIZE}) push=${PUSH_STRATEGY} run ${x}/${RUNS}: already have ${OUT_FILE}, skipping ==="
                continue
            fi
            SEED=$(( x * 100 ))
            echo "=== ${DATA_FILE_NAME} (pool_size=${POOL_SIZE}) push=${PUSH_STRATEGY} run ${x}/${RUNS}: master-slave, ${NUM_WORKERS} MPI ranks (1 master + $((NUM_WORKERS - 1)) workers), base seed ${SEED} ==="
