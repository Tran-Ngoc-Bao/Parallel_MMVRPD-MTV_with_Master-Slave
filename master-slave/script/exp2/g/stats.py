...
Reports 5 numbers per (pool_set, strategy) pair, printed as one table per
...
a nuisance parameter to marginalize out):
  ...
  - PoolDiv: elite_pool_diversity held at program end (mean pairwise
...
Aggregation: same stratified style as exp2/f for final_rpd, irpd, and
PoolDiv -- computed per run first (using that run's own solution/
checkpoints/elite_pool_diversity vs BKS), then averaged over that
instance's runs, then averaged over instances (4 per pool_set: 2 customer
counts x 2 combos). AR and ER instead follow exp2/c's pooling: N_accept
summed over the instance's runs / N_offer (or N_accept, for ER) summed
over the instance's runs * 100 (not averaged per-run), since offer/accept
counts can vary a lot run to run -- pooling weights each pull attempt
equally instead of each run equally. These per-instance numbers are then
averaged over instances to get each (pool_set, strategy) pair's final
numbers.
        ...
        for strategy in REPLACE_STRATEGY_LIST:
            ...
            for run_id, path in run_files:
                ...
                detail_rows.append({
                    ...
                    "pool_diversity": run_pool_diversity,
                })
...
def main():
    ...
    for pool_set in POOL_SET_NAMES:
        ...
        detail_header = (f"{'Instance':<12}{'Strategy':<14}{'Runs':>6}"
                          f"{'final_RPD(%)':>14}{'irpd(%)':>10}{'AR(%)':>10}{'ER(%)':>10}{'PoolDiv':>10}")
        out(detail_header)
        ...
        for r in summary_rows:
            ...
            out(f"{r['instance']:<12}{r['strategy']:<14}{r['runs']:>6}"
                ...
                f"{fmt(r['ar_pct'], 3):>10}{fmt(r['er_pct'], 3):>10}{fmt(r['pool_diversity'], 3):>10}")

        # Summary: mean over instances, one row per strategy.
        out(f"\n{'Strategy':<14}{'final_RPD(%)':>14}{'irpd(%)':>10}{'AR(%)':>10}{'ER(%)':>10}{'PoolDiv':>10}")
        out("-" * 68)
        for strategy in REPLACE_STRATEGY_LIST:
