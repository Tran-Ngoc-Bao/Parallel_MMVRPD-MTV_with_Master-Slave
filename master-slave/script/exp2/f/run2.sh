...
OUTPUT_DIR="${OUTPUTS_DIR:-${SCRIPT_DIR}/../../../outputs/exp2/f/${DATA_PREFIX}}"
mkdir -p "${OUTPUT_DIR}"
...
for DATA_FILE in "${DATA_FILES[@]}"; do
    ...
    for REPLACE_STRATEGY in "${REPLACE_STRATEGY_LIST[@]}"; do
        for ((x = 1; x <= RUNS; x++)); do
            RUN_ID="repl${REPLACE_STRATEGY}-${x}"
            OUT_FILE="${OUTPUT_DIR}/${DATA_FILE_NAME}-${RUN_ID}.json"
            if [ -f "${OUT_FILE}" ]; then
                echo "=== ${DATA_FILE_NAME} replace=${REPLACE_STRATEGY} run ${x}/${RUNS}: already have ${OUT_FILE}, skipping ==="
                continue
            fi
            SEED=$(( x * 100 ))
