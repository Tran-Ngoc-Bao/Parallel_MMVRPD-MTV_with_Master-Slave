#!/usr/bin/env python3
"""
Emit a long/tidy CSV from master-slave exp1-full output for plotting
convergence curves (best cost at each time checkpoint).

Source:
    <outputs-dir>/<method>/<customers>/<customers>.<combo>-<run>.json
Each file must carry "best_solution_cost_by_time_checkpoint" (a 9-element
array, index 0..8 -- written by the master when run with --time-limit).
Files without it / with an empty array are skipped.
...
One CSV row per (instance, run, method, checkpoint):
    ...
    method       group directory name (default: coop)
    checkpoint   0..8
    q            checkpoint as a fraction: 0 -> "0/8", 1 -> "1/8", ...
    BestCost     best_solution_cost_by_time_checkpoint[checkpoint] (not rounded)
    BKS          BKS working_time (not rounded)

Usage:
    python3 stat_checkpoints.py
    python3 stat_checkpoints.py --methods coop --customers 500,1000
...
CHECKPOINT_FIELD = "best_solution_cost_by_time_checkpoint"
    ...
    for method in methods:
        ...
        for n in discover_customers(method_dir):
            ...
            for fpath in sorted(cust_dir.glob("*.json")):
                ...
                series = data.get(CHECKPOINT_FIELD) or []
                if not series:
                    skipped_no_field += 1
                    continue
...
def main():
    ...
    ap.add_argument("--methods", nargs="+", default=["coop"],
                    help="Cac method (ten thu muc group) can gom (mac dinh: coop)")
    ...
    if stats["skipped_no_field"]:
        print(f"  bo qua {stats['skipped_no_field']} file thieu "
              f"'{CHECKPOINT_FIELD}' (chay khong co --time-limit?)")
