#!/usr/bin/env python3
"""
Statistics for run2_ims.sh (exp1.5) output.

Unlike exp1's stats.py, this is ims-only (no sats counterpart to compare
against, no CV/delta/win columns) and reports two views per instance
(n.combo):
...
RPD (%) = (working_time - BKS) / BKS * 100, using the working_time of the
best worker kept for that run (run2_ims.sh already keeps only the best of
N parallel workers per run).
...
by run2_ims.sh) and the matching BKS file at <bks>/<n>/<n>.<combo>-bks.json.
...
def main():
    ...
    ap.add_argument("--outputs", default="../../outputs/exp1.5",
                     help="Dir containing run2_ims.sh output, relative to this script "
    ...
    ap.add_argument("--combos", default="10.2,40.1",
                     help="Comma-separated instance combos, matches run2_ims.sh "
                          "(default: 10.2,40.1)")
